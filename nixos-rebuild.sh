...
# Edit your config
nvim /home/shim/dotfiles/nixos/

# cd to your config dir
pushd /home/shim/dotfiles/nixos/
...
# Rebuild system config, output simplified errors, log trackebacks
sudo nixos-rebuild switch --flake .#phoenix &>nixos-switch.log || (cat nixos-switch.log) # | grep --color error && exit 1)

# Rebuild home
sudo -u shim -i home-manager switch --flake /home/shim/dotfiles/nixos#shim

# Get current generation metadata
current=$(nixos-rebuild list-generations | grep current)

# Commit all changes witih the generation metadata
git -c user.name="KshitijKapoor8" -c user.email="kshitijkapoor8@gmail.com" commit -am "$current"
